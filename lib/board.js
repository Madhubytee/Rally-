...
/**
 * `9 AM` / `1:30 PM` from a 24-hour `<input type="time">` value.
 * Seed events read "Sunday 8 AM", so a published one must not read "09:00".
 */
export function formatEventTime(value) {
  const [h, m] = (value || DEFAULT_START_TIME).split(':').map(Number)
...
export function publishDraft(draft) {
  ...
  return {
    ...
    title: draft.name,
    when: `${formatEventDate(date)} ${formatEventTime(time)}`,
    going: 1,
