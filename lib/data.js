...
/**
 * Database ids are uuids; bundled seed ids are readable strings like `r1`
 * and `globe-50490`.
 *
 * Both can be on screen at once — the app renders seed data until the tables
 * are populated — so anything about to be written to a uuid column has to
 * check. Passing `r1` to a uuid foreign key is not a soft failure; Postgres
 * rejects the whole insert and the publish dies with a type error the user
 * cannot act on.
 */
const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i
export const isPersistedId = (id) => typeof id === 'string' && UUID.test(id)
...
export async function createEvent(draft, { userId, hostName }) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      /*
       * Null when the pin came from the bundled seed rather than the
       * database. The event is still worth publishing — it carries its own
       * location and supplies — it just cannot point back at a row that was
       * never inserted.
       */
      issue_id: isPersistedId(draft.issueId) ? draft.issueId : null,
      organizer_id: userId,
      title: draft.name,
      loc: draft.loc,
      lat: draft.lat,
...
export async function createSignup(eventId, { name, email, phone, org, party }) {
  ...
  /* Same guard: a seeded event has no row to attach a signup to. */
  if (!isPersistedId(eventId)) return null
